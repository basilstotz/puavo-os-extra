...
echo "deb http://download.zerotier.com/debian/bullseye bullseye main" >/tmp/zt-sources-list
mv -f /tmp/zt-sources-list /etc/apt/sources.list.d/zerotier.list

apt-get update
apt-get install -y zerotier-one

sed -e"s@ExecStart=/usr/sbin/zerotier-one@ExecStart=/usr/sbin/zerotier-one /images/amxa/zerotier-home@g" -i /lib/systemd/system/zerotier-one.service

systemctl enable zerotier-one
#systemctl start zerotier-one

if test -d /images/; then
   mkdir -p /images/amxa/zerotier-home
fi
...
NETWORK=$(puavo-conf puavo.zerotier.network 2>/dev/null)

if test -n "$NETWORK"; then
   if test -d /images/amxa/zerotier-home; then
       zerotier-cli -D/images/amxa/zerotier-home join "$NETWORK"
   fi
fi
EOF
